...
struct MeshCache {
    ...
    pub meshes: Vec<Option<Arc<(Vec<Vertex>, Vec<u16>)>>>,
...
impl MeshCache {
    ...
    pub fn get_mesh(&self, x: i32, y: i32, z: i32) -> Option<Arc<(Vec<Vertex>, Vec<u16>)>> {
        ...
        self.meshes[z * size * size + y * size + x].clone()
    }
    ...
    fn init(&mut self) {
        self.terrain_worker = T::new(
            Arc::new(Mutex::new({
                ...
                move || {
                    ...
                    if let Some((x, y, z)) = result {
                        map_cache.chunk_loading.insert((x, y, z));
                    }
                    result
                }
            })),
            ...
            Arc::new(Mutex::new({
                ...
                move || {
                    let mut mesh_cache = mesh_cache.lock().unwrap();
                    mesh_cache.mesh_load_request.pop_front()
                }
            })),
            Arc::new(Mutex::new({
                let mesh_cache = self.mesh_cache.clone();
